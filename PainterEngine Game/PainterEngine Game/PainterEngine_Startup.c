...
px_uint32 Game_rand()
{
	return (px_uint32)(game_rand_seed=PX_randEx(game_rand_seed))%0xffff;
}
...
px_void PX_PlayLocalSoundFromResource(PX_Instance *pIns,PX_World *pWorld,px_float x,px_float y,const px_char key[],px_bool loop)
{
	...
	if(!PX_isPointInRect(PX_POINT(x,y,0),PX_RECT((px_float)pWorld->offsetx-128,(px_float)pWorld->offsety-128,(px_float)pWorld->surface_width+256,(px_float)pWorld->surface_height+256)))
		return;
