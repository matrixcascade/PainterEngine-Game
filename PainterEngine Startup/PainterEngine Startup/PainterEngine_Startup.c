...
{
	FILE *pf_res=PX_NULL,*pf_dest=PX_NULL;
	pf_res=fopen(resFile,"rb");
	if(!pf_res) goto _ERROR;
	pf_dest=fopen(DestFile,"wb");
	if(!pf_dest) goto _ERROR;

	while (!feof(pf_res))
	{
		px_int readsize;
		px_byte cache[1024];
		readsize=fread(cache,1,sizeof(cache),pf_res);
		fwrite(cache,1,readsize,pf_dest);
	}
	fclose(pf_res);
	fclose(pf_dest);
	return PX_TRUE;
